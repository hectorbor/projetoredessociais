...
file_path = '../stored_dataframes/game3939969.csv'
...
exgrapM= [[0 for _ in range(11)] for _ in range(11)]

for matrix in team2.matrices:
    for i in range(11):
        for j in range(11):
            exgrapM[i][j] += matrix[i][j]
...
# Dramatic size scaling using exponential, minimum size plus exponential growth
node_sizes = [300 + (degrees[node] ** 3) * 200 for node in G.nodes()]
